...
def send_to_discord_webhook(webhook_url=None, message=None):
    data = {
        "content": message
    }
    headers = {
    ...
    webhook_url = os.getenv("test_webhook")
...
def status():
    ...
    for entry in heartbeat_list:
        # Parse the UTC time string according to the provided format
        utc_time = datetime.strptime(entry["time"], "%Y-%m-%d %H:%M:%S.%f")
        
        # Manually convert UTC to IST by adding 5 hours and 30 minutes
        ist_time = utc_time + timedelta(hours=5, minutes=30)
        ist_time_str = ist_time.strftime("%Y-%m-%d %H:%M:%S.%f")

        # Append the data with the IST time replacing the original time
        status_data.append({
            "time": ist_time_str,
            "status": entry["status"],
            "ping": entry["ping"]
...
def record_accurate():
    ...
    if mmr_history_data['status'] == 200:
        ...
        mmr_list.append(mmr_history_data['data'][c])    
        total_mmr_change = mmr_list[0]['elo'] - mmr_list[-1]['elo'] 
        ...
        # Handle error fetching MMR history
        return "Error fetching MMR history from Riot API"
    mmr_list = json.dumps(mmr_list, indent=2)
    print(mmr_list)
    return f"{streamer_name} is {up_or_down} {total_mmr_change} RR, with {win} wins, {lose} losses and {draw} draws this stream.."
